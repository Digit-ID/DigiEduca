-- MIGRAÇÃO DigiEduca → projeto Supabase do DigiNote (correr UMA vez, tudo de seguida)
...
-- =====================================================================
--  Contas já existentes do DigiNote/DigiSpot → criar perfil DigiEduca
--  (novos registos ficam cobertos pelo gatilho trg_novo_utilizador)
-- =====================================================================
insert into public.perfis (id, email, nome, departamento, papel)
select u.id, u.email,
  ...
  coalesce(u.raw_user_meta_data->>'departamento', 'Por definir'),
  'colaborador'
from auth.users u
where u.email ilike '%@digit.com.pt'
  and not exists (select 1 from public.perfis p where p.id = u.id);

-- Conta de administração
update public.perfis set papel = 'admin' where email = 'id@digit.com.pt';

-- Confirmação: perfis criados e estados dos cursos
select (select count(*) from public.perfis) as perfis,
       (select count(*) from public.cursos where estado = 'disponivel') as cursos_disponiveis;
